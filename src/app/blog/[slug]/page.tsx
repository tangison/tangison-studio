          ...
          <div className="mx-auto max-w-2xl px-6">
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {article.tags.map((tag) => (
                <span key={tag} className="inline-flex items-center px-2.5 py-1 rounded-[25px] text-xs font-medium bg-ocean-mist text-ink">#{tag}</span>
              ))}
            </div>
          </div>
        </section>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {related.map((rel) => (
                  <Link key={rel.slug} href={`/blog/${rel.slug}`} className="group block">
                    <div className="aspect-[3/2] overflow-hidden rounded-[25px] bg-ocean-mist mb-3">
