...
export function StudioSearch({ isOpen, onClose, items }: StudioSearchProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[110] flex items-start justify-center pt-[12vh] px-4" role="dialog" aria-modal="true" aria-label="Search">
      ...
      <div className="relative w-full max-w-2xl bg-skeleton-bone rounded-[24px] border border-card-border shadow-2xl overflow-hidden">
            ...
            Object.entries(grouped).map(([category, categoryItems]) => (
              <div key={category} className="mb-2">
                ...
                {categoryItems.map((item) => {
                  ...
                  return (
                    <Link key={item.href} href={item.href} onClick={onClose} className={`flex items-center justify-between px-4 py-3 rounded-[12px] transition-colors ${isSelected ? "bg-ocean-mist" : "hover:bg-ocean-mist/50"}`}>
                      <div>
